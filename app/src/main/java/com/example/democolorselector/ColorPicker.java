...
public class ColorPicker extends View {
     ...
     */
    private com.example.democolorselector.SaturationBar mSaturationBar = null;
     ...
     */
    private com.example.democolorselector.ValueBar mValueBar = null;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        mPointerColor.setColor(getResources().getColor(R.color.white));
    ...
    private String calculateColorSendToListener(float angle) {
        float unit = (float) (angle / (2 * Math.PI));
        if (unit < 0) {
            unit += 1;
        }
        ...
        if (unit <= 0) {
            mColor = COLORS[0];
            return String.valueOf(COLORS[0]);
        }
        if (unit >= 1) {
            mColor = COLORS[COLORS.length - 1];
            return String.valueOf(COLORS[COLORS.length - 1]);
        }
        ...
        rgb.append(r).append(" ").append(g).append(" ").append(b);
        return rgb.toString();
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (onColorSelectedListener != null && mCenterNewColor != oldSelectedListenerColor) {
                    onColorSelectedListener.onColorSelected(calculateColorSendToListener(mAngle));
